function [ Ps_sim_4QAM,Ps_exact_4QAM,Ps_nearest_4QAM,Eb_No_db] = fun4qam( ~ )
...
M = 4;
d = 1;
constellation_point = [ -d+j*1*d  d+j*1*d  -d-j*1*d  d-j*1*d];
...
Ps_sim_4QAM(L) = number_of_symbol_error/N;
Ps_exact_4QAM(L) = 1-(1- 2*(sqrt(M)-1)/sqrt(M)*qfunc(sqrt(3*Eb_No_linear*log2(M)/(M-1))))^2;
Ps_nearest_4QAM(L) = (4*(sqrt(M)-1)/sqrt(M))*qfunc(sqrt((3*Es/N0)/(M-1)));
end
